...
* **Eventos (Agenda unificada)**
  ...
  * **Recorrência** (mínimo viável e robusto):
    ...
    * `semanal`: dia\_da\_semana (dom..sáb), hora.
    * `mensal`: “n-ésima semana + dia da semana” (ex.: 1ª sexta, 3º domingo) + hora.
    * `único`: data e hora específicas.
  * **Janela de validade**: data\_inicio (obrigatória para recorrentes), data\_fim (opcional).
  * **Exceções**: coleção **eventos\_excecoes** (data específica marcada como cancelada/alterada, com observação).
...
* **Consentimento**: flags “exibir\_whatsapp\_publicamente” por pessoa/entidade.
...
* Agendas recorrentes **sempre** com data\_inicio e hora.
...
* Para **semanal**: próximo dia\_da\_semana ≥ hoje; se hoje e hora já passou, rola para a próxima semana.
...
* **Janela de validade**: respeitar data\_inicio/data\_fim.
